...
var cfgFile string
var roorccored = &cobra.Command{
	Use:   "rccore",
	Short: "rccore - Tom's Cluster Manager",
	Long:  ``,
...
func init() {
	roorccored.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.together/cfg.yaml)")
	roorccored.AddCommand(starrccored)
	roorccored.AddCommand(versionCmd)
	roorccored.AddCommand(updateCmd)
	roorccored.AddCommand(clusterCmd)
...
func Execute() {
	if err := roorccored.Execute(); err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
}

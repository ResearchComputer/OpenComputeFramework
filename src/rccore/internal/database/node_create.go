...
func (nc *NodeCreate) SetPeerId(s string) *NodeCreate {
	nc.mutation.SetPeerId(s)
	return nc
}

// SetNillablePeerId sets the "peerId" field if the given value is not nil.
func (nc *NodeCreate) SetNillablePeerId(s *string) *NodeCreate {
	if s != nil {
		nc.SetPeerId(*s)
	}
	return nc
...
func (nc *NodeCreate) defaults() {
	if _, ok := nc.mutation.PeerId(); !ok {
		v := node.DefaultPeerId
		nc.mutation.SetPeerId(v)
	}
...
func (nc *NodeCreate) check() error {
	if _, ok := nc.mutation.PeerId(); !ok {
		return &ValidationError{Name: "peerId", err: errors.New(`database: missing required field "Node.peerId"`)}
	}
	if _, ok := nc.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`database: missing required field "Node.status"`)}
	}
	return nil
}
...
func (nc *NodeCreate) createSpec() (*Node, *sqlgraph.CreateSpec) {
	...
	if value, ok := nc.mutation.PeerId(); ok {

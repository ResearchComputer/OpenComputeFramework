...
func StartTicker() {
	...
	common.ReportError(err, "Error while creating reconnect ticker")
			...
			if error == nil {
				if host.Network().Connectedness(peer_id) == network.Connected {
					...
					if len(addrInfo.Addrs) == 0 {
						//common.Logger.Infof("No known addresses for peer %s; marking disconnected", peer_id)
						p.Connected = false
						disconnected++
					} else if err := host.Connect(ctx, addrInfo); err != nil {
						//common.Logger.Infof("Failed to dial peer %s; marking disconnected", peer_id)
						p.Connected = false
						disconnected++
					} else {

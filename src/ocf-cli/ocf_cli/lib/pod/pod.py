...
from ocf_cli.lib.pod.utils import allow_missing, logger
...
class Pod(object):
    def __init__(
        ...
        *,
        ...
        self._pod_path = pod_path
        self._hid = os.path.basename(pod_path)
        self._env = (
            dict(os.environ, **additional_env) if additional_env else dict(os.environ)
        )
        self._pid_file = pod_path / "pid"
        self._rc_file = pod_path / "rc"
        self._name_file = pod_path / "name"
        self._cmd_file = pod_path / "cmd"
        self._env_file = pod_path / "env"
    ...
    def _set_name(self, name: Optional[str]):
        """
        Sets name for the first time on pod creation.
        """
        if name is None:
    ...
    def _set_cmd(self, cmd: Optional[str]):
        """
        Sets cmd for the first tiem on pod creation.
        """
        if self.cmd is None:
            if cmd is None:
    ...
    def attach(self, pid: int):
        """
        Associate pod object with existing process by pid
        """
        try:
            self._set_pid(pid)
            self._set_env()
    ...
    def start_time(self) -> Optional[str]:
        ...
        logger.info("No start time as pod has not started yet")
    ...
    def end_time(self) -> Optional[str]:
        ...
        logger.info("No end time as pod has not finished yet")
    ...
    def stdout_path(self):
        return self._pod_path / "stdout.log"

    @property
    def stderr_path(self):
        return self._pod_path / "stderr.log"
    ...
    def __rich__(self) -> str:
        ...
        rich_text = (
            f"pod {config.ICON_POD}{self.hid} "
